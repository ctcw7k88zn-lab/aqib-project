...
def _ensure_nltk_stopwords():
    """
    Avoid slow / failing downloads on every app start.
    If stopwords aren't present, try to download once (quietly).
    """
    try:
        ...
        try:
            ...
            # App can still run (resume parsing quality may degrade).
            pass
...
# sql connector
DB_ENABLED = os.getenv("RESUME_DB_ENABLED", "1") != "0"
SHOW_DB_ERROR = os.getenv("RESUME_SHOW_DB_ERROR", "0") == "1"
DB_ERROR = None
connection = None
cursor = None
...
if DB_ENABLED:
    try:
        connection = pymysql.connect(
            ...
            password=os.getenv("RESUME_DB_PASSWORD", "root@MySQL4admin"),
...
def insert_data(sec_token,ip_add,host_name,dev_user,os_name_ver,latlong,city,state,country,act_name,act_mail,act_mob,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses,pdf_name):
    if not DB_ENABLED or connection is None or cursor is None:
        return
...
def run():
    ...
    # DB UX:
    # - User mode should still work without DB (we'll save nothing).
    # - Admin/Feedback require DB; if missing, show one clear message and stop.
    if not DB_ENABLED:
        if choice in ("Admin", "Feedback"):
            st.warning("MySQL database is not running/configured, so Admin/Feedback analytics are disabled.")
            if SHOW_DB_ERROR and DB_ERROR is not None:
                st.caption(f"DB error: {DB_ERROR}")
            return
        elif choice == "User":
            st.sidebar.info("MySQL DB is not running: User mode works, but results won't be saved.")
    else:
    ...
    if choice == 'User':
        ...
        try:
            ...
            if latlong:
                ...
                address = location.raw.get('address', {}) if location and location.raw else {}
                city = address.get('city', '') or address.get('town', '') or address.get('village', '')
                state = address.get('state', '')
                country = address.get('country', '')
        except Exception:
            # Network / geocoding can fail; keep fields blank.
            pass
